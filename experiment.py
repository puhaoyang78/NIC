...
def parse_models(value):
    if value == 'all':
        return MODES
    values = tuple(dict.fromkeys(x.strip() for x in value.split(',') if x.strip()))
    invalid = [x for x in values if x not in MODES]
    if not values or invalid:
        raise argparse.ArgumentTypeError(f'Unknown models: {invalid}; choices are {MODES}')
    return values
...
def inspect_data(args):
    ...
    for path in files:
        ...
        write_json(Path(args.output) / 'inventory.json', dict(
            data_root=str(root), sessions=rows,
...
def make_splits(rows, seed):
    """Leave one complete session out for test; keep validation session-disjoint."""
...
def cache_directory(args):
    out = Path(args.output) / f'window_{args.window_seconds:g}s'
    return out / ('smoke_cache' if args.smoke else 'cache')


def resample_ratio(source_n, target_n, max_denominator=512):
    """Bound the polyphase ratio so unusual Pico rates do not create huge FIR filters."""
    ratio = Fraction(target_n, source_n).limit_denominator(max_denominator)
...
def prepare(args):
    ...
    for idx, row in enumerate(rows):
        ...
        up, down = resample_ratio(source_n, target_n)
        ...
        with reader:
            for chunk in reader:
                ...
                if count:
                    ...
                    if valid.any():
                        transformed = resample_poly(raw[valid], up, down, axis=1, window=fir)
                        if transformed.shape[1] > target_n:
                            transformed = transformed[:, :target_n]
                        elif transformed.shape[1] < target_n:
                            transformed = np.pad(transformed, ((0, 0), (0, target_n - transformed.shape[1])), mode='edge')
                        if transformed.shape != (int(valid.sum()), target_n):
                            raise AssertionError('Unexpected resampled shape')
        ...
        print(f'Prepared {row["session"]}: {used - len(excluded)}/{n_windows} valid windows; '
              f'excluded={len(excluded)}, resample={up}/{down}', flush=True)
...
def metric(y, probabilities, n):
    ...
    p, r, f, _ = precision_recall_fscore_support(
        y, pred, labels=list(range(n)), average='macro', zero_division=0)
...
def run(args):
    ...
    need_features = any(mode in ('rf', 'rf_frequency') for mode in args.models)
    need_frequency = 'rf_frequency' in args.models
    all_features = None
    frequency_names = []
    if need_features:
    ...
    class CNN(nn.Module):
        def __init__(self, mode, n_classes):
            ...
            if mode == 'stft_cnn':
                ...
                self.body = nn.Sequential(
                    nn.Conv1d(1, 16, 9, stride=4, padding=4), nn.ReLU(),
            ...
            for mode in args.models:
                ...
                if mode in ('rf', 'rf_frequency'):
                    model = RandomForestClassifier(
                        n_estimators=10 if args.smoke else 100,
                    ...
                    val_scores = metric(ys['val'], model.predict_proba(all_features[batch_ids['val'].cpu().numpy(), :width]), n)
                    import joblib
                    ...
                    for epoch in range(epochs):
                        ...
                        history.append(dict(epoch=epoch + 1, train_loss=train_loss,
                                            val_loss=val_loss, epoch_seconds=epoch_seconds))
                        print(f'fold={fold_id} {task} {mode} epoch={epoch + 1} '
                              f'train_loss={train_loss:.5f} val_loss={val_loss:.5f} seconds={epoch_seconds:.2f}', flush=True)
                ...
                for aggregation in args.aggregations:
                    ...
                    for session in split['test']:
                        ...
                        ends, averaged = aggregate_probabilities(
                            indices[session], probabilities[pos:pos + size], aggregation)
                        ...
                        for j, end in enumerate(ends):
                            predictions.append(dict(
                                ...
                                end_window=int(indices[session][end]),
                                true_label=names[truth[j]], predicted_label=names[averaged[j].argmax()],
                    ...
                    scores = dict(
                        ...
                        test_sessions=split['test'], trained_epochs=epochs,
                    ...
                    if predictions:
                        ...
                        prediction_path.write_text(
                            'session,start_window,end_window,true_label,predicted_label,' +
                            ','.join(f'probability_{name}' for name in names) + '\n')
                    if not binary:
                        save_cm(Path(str(prefix) + f'_aggregate{aggregation}_confusion.csv'),
                                truth, prob.argmax(axis=1), names)
                    pooled.setdefault((task, mode, aggregation), []).append((truth, prob))
                print(f'fold={fold_id} {task} {mode} completed', flush=True)
    ...
    for (task, mode, aggregation), chunks in pooled.items():
        ...
        for key in pooled_scores:
            ...
            diagnostic[key] = dict(mean=float(np.mean(values)) if values else None,
                                   std=float(np.std(values)) if values else None,
                                   valid_folds=len(values))
        session_accuracy = [r['accuracy'] for r in group if r['accuracy'] is not None]
        summary.append(dict(
            task=task, model=mode, aggregation=aggregation,
            observation_seconds=aggregation * args.window_seconds,
...
def main():
    ...
    prepare_parser = commands.add_parser('prepare', help='Split sessions then extract non-overlapping windows')
    ...
    run_parser.add_argument('--aggregations', type=parse_int_list, default=(1,),
                            help='Comma-separated consecutive-window counts; default 1')
